    ...
    <section id="projects" ref={container} className="py-20 px-10 bg-[#0c0c0c]">
      <div className="flex justify-between items-end mb-16 border-b border-plum/30 pb-10">
        <h2 className="text-6xl font-bold">
          Work<span className="text-plum">.</span>
        </h2>
        <div className="flex gap-4">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-full text-xs uppercase border transition-all ${
                filter === cat
                  ? "bg-plum border-lavender"
                  : "border-orchid/30 text-orchid"
              }`}
            >
              {cat}
            </button>
